...
<head>
    ...
    <title>Appointment Management - Medic AI</title>
    <style>
        .badge {
            font-size: 0.8rem;
            font-weight: 500;
...
<body>
    ...
    <div class="container py-5">
        <div class="row mb-4">
            <div class="col-md-8">
                <h1 class="h3 mb-2">Appointment Management</h1>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($appointment['status'] === 'pending'): ?>
                                            ...
                                            <button 
                                                class="btn btn-sm btn-success btn-action confirm-btn" 
                                            ...
                                            <?php elseif($appointment['status'] === 'confirmed'): ?>
                                                <?php if(empty($appointment['doctor_id']) || $appointment['doctor_id'] == $doctorId): ?>
                                                    <!-- Only show complete button if the appointment was confirmed by this doctor or has no doctor assigned -->
                                                    <button 
                                                    ...
                                                    <!-- Cancel button for doctors who confirmed the appointment -->
                                                    <button 
                                                ...
                                                <?php else: ?>
                                                    <!-- Show disabled buttons for appointments confirmed by other doctors -->
                                                    <button class="btn btn-sm btn-outline-secondary" disabled title="Confirmed by another doctor">
                                                        <i class="bi bi-check-all me-1"></i> Cannot Complete
                                                    </button>
                                                    
                                                    <button class="btn btn-sm btn-outline-secondary" disabled title="Confirmed by another doctor">
                                                        <i class="bi bi-x-circle me-1"></i> Cannot Cancel
                                                    </button>
                                                <?php endif; ?>
    ...
    <form id="confirmForm" action="../../Controller/doctorAppointmentController.php" method="post" style="display:none;">
        <input type="hidden" name="action" value="confirm">
        <input type="hidden" name="id" id="confirm_appointment_id">
        ...
        $(document).ready(function() {
            ...
            $('.confirm-btn').click(function() {
                ...
                Swal.fire({
                    ...
                    html: `Are you sure you want to confirm the appointment for <strong>${patientName}</strong> on <strong>${appointmentDate}</strong> at <strong>${appointmentTime}</strong>?`,
                    icon: 'question',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $('#confirm_appointment_id').val(appointmentId);
                        $('#confirmForm').submit();
                    }
                });
            });
